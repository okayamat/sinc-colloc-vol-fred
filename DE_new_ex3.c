...
double kDE1(double t, double a, double b, double tau)
{
  ...
  return 0.5 * M_1_PI * t * exp(-s) * (t - s) * sqrt(2.0/(1 + exp(-M_PI*sinh(tau))));
}
...
{
  double s = DE_trans(a, b, tau);
  return -M_1_PI * (s + t*t) / cosh(M_PI_2*sinh(tau));
}
...
double gDE(double a, double b, double tau)
{
  ...
  double val = sqrt(2.0/(1 + exp(-M_PI*sinh(tau))));

  val -= t * 0.5 * M_1_PI
         * ( (3 + t)*exp(-t) - M_E * 2 / (1 + exp(M_PI*sinh(tau))) );
